...
if ( ! class_exists( 'BpMembersTab' ) ):
	class BpMembersTab {
		...
		public function title() {
			...
			return '<dd class="' . $active . '">' .
			       '<a href="#' . esc_attr( str_replace( "%", "", sanitize_title_with_dashes( $this->args["name"] ) ) ) . '">' .
			       sweet_translate_dynamic( $this->args["name"], 'sweetdate' ) .
			       '</a>' .
			       '</dd>';
		}
						...
						if ( bp_profile_group_has_fields() ) :
							...
							while ( bp_profile_fields() ) : bp_the_profile_field();
								if ( bp_field_has_data() ) :
									...
									$this->tabs_instance->fields_data[ $name ] .= '<dd class="bp-field-value bp-field-id-' . esc_attr( bp_get_the_profile_field_id() ) . '">' . sweet_translate_dynamic( bp_get_the_profile_field_value(), 'sweetdate' ) . '</dd>';
								endif;
							endwhile;
		...
		public function has_profile_data( $name ) {
			...
			if ( bp_album_has_pictures() ) :
                ...
                <div class="four columns centered btn-carousel hide-for-small">
                  <small><a href="#" id="left-prev">' . esc_html__( "PREVIOUS", 'sweetdate' ) . '</a>&nbsp; &nbsp; &nbsp;
                  <a href="#" id="right-next">' . esc_html__( "NEXT", 'sweetdate' ) . '</a></small>
                </div>';
	...
	class BpMembersTab_rt_media extends BpMembersTab {
		...
		public function has_profile_data( $name ) {
			...
			if ( $myrows && count( $myrows ) > 0 ) {
				...
				$this->tabs_instance->fields_data[ $name ] .= '<div class="clearfix"></div>
						<div class="four columns centered btn-carousel hide-for-small">
							<small><a href="#" id="rthumb-prev">' . esc_html__( "PREVIOUS", 'sweetdate' ) . '</a>&nbsp; &nbsp; &nbsp;
							<a href="#" id="rthumb-next">' . esc_html__( "NEXT", 'sweetdate' ) . '</a></small>
						</div>';
			}
